...
For a small feedback set, balance the classes before training:

```bash
node scripts/export-training-data.js --min-confidence 3 --balance
```
...
**What it does:**
1. Reads `feedback-data.jsonl` entries with manual user feedback by default (automatic heuristic feedback remains available for analysis but is excluded as a model label)
...
5. Optionally balances the classes deterministically with `--balance`
6. Outputs to `training-data.jsonl`

Use `--include-agent-auto` only for experiments. Automatic feedback is generated by the current heuristic scorer, so including it can create a self-reinforcing training loop rather than adding independent preference labels.
